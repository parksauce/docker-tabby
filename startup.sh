...
# Get Variables
read -ep "Enter a path to store your configuration files for tabby (mandatory): \n" TABBY_PATH
echo ''
read -ep "Enter a password for the database (leave blank to generate one at random): \n" DB_PASSWORD
echo ''
id
read -ep "Enter your UID and GID separated by a space (optional, use the values above): \n" PUID PGID
echo ''
read -ep "Enter you timezone (optional, ex. America/New_York): " TZ
echo ''
read -ep "Enter a version for Tabby (if none is set latest will be used): \n" VERSION

# Get Mail server variables
echo ''
echo '\nMoving on to mail server configuration'
echo ''
read -ep "Enter your mail server's host. (Example: mail.example.com) \n" TABBY_SMTP_HOST
echo ''
read -ep "Enter you mail server's port. Typically, 465 or 587. (Default: 465): \n" TABBY_SMTP_PORT
echo ''
read -ep "Enter your SMTP server's authentication method. Typically, PLAIN, LOGIN, CRAM-MD5. Any of those options should be entered in plane text and will be encoded on launch, any other methods should be hashed manually. (Default: LOGIN): \n" TABBY_SMTP_AUTH_METHOD
echo ''
read -ep "Enter your SMTP server's user. (Example: user@example.com): \n" TABBY_SMTP_USER
echo ''
read -ep "Enter your SMTP user's password. (Example: MyPassword): \n" TABBY_SMTP_PASS
echo ''
read -ep "Enter Yes or No to use TLS. (Default: Yes): \n" TABBY_SMTP_USE_TLS
echo ''
read -ep "Enter Yes or No to use STARTTLS. (Default: No): \n" TABBY_SMTP_USE_STARTTLS
